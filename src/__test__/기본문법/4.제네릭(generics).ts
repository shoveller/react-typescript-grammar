 ...
 * 아래 타입의 에러를 고치세요.
 */
const 성공: 서버리스폰스<string> = {
  data: "데이터",
  code: 200,
};

const 성공1: 서버리스폰스1<string> = {
  data: "0",
  code: 200,
};
 ...
 */
const 뒤집기함수 = <대충퉁침>(배열: 대충퉁침[]) => {
  return 배열.reverse();
};
function 뒤집기함수2<대충퉁침>(배열: 대충퉁침[]) {
  return 배열.reverse();
}
console.log(뒤집기함수([1, 2, 3]));
console.log(뒤집기함수2(["1", "2", "3"]));

/**
 * 제네릭 함수는 extends 키워드로 제약조건을 걸 수 있습니다.
 * 아래의 제네릭 함수에는 length 라는 프로퍼티를 가진 객체만 들어갈 수 있습니다.
 */
const 갯수세기 = <대충퉁침 extends { length: number }>(객체: 대충퉁침) => {
  return 객체.length;
};

console.log(갯수세기("123"));
console.log(갯수세기([1, 2, 3]));
console.log(갯수세기({ length: 0 }));
...
console.log(갯수세기1([1, 1, 2, 2]));
console.log(갯수세기1({ length: 0, 0: 1, 1: 1 }));

/**
 * 문제)
 * { a: string } 라는 프로퍼티를 포함하는 객체만 허용하는 제네릭함수를 선언해 보세요.
 */
